...
export const useFileList = () => {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return (await (actor as any).listFileReferences()) as FileReference[];
    },
    enabled: !!actor,
...
export const useFileDelete = () => {
  ...
  const deleteFile = async (path: string): Promise<void> => {
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (actor as any).dropFileReference(path);
      await invalidateFileList();
...
export const useInvalidateQueries = () => {
  ...
  return {
    ...
    invalidateFileUrl: (filePath: string) =>
      queryClient.invalidateQueries({ queryKey: ["fileUrl", filePath] }),
    invalidateAll: () => {
      queryClient.invalidateQueries({ queryKey: ["fileList"] });
